...
type TypingStartEvent = ChatEvent<'typing:start'>;
// { type, timestamp, conversationId, participant, typing }

type TypingStopEvent = ChatEvent<'typing:stop'>;
// { type, timestamp, conversationId, participant, typing, reason }
```

Typing events are conversation-scoped and ephemeral. Repeated inbound `typing:start`
updates refresh TTL in core without re-emitting duplicate start events for the same
participant.
