...
function engineInternals(engine: IChatEngine): EngineInternals {
  ...
  return {
    ...
    transport:
      transport && typeof transport['getState'] === 'function'
        ? (transport as unknown as EngineInternals['transport'])
        : undefined,
  };
}
...
function summarizeEvent(
  ...
  const payloadRecord = isRecord(payload) ? (payload as Record<string, unknown>) : undefined;
...
export function useChatDevTools(
  ...
  useEffect(() => {
    ...
    const unsubscribeStreamEnd = engine.on('message:stream:end', (event) => {
      ...
      setStreams((current) => {
        const nextStreams = current.map((stream): StreamSnapshot => {
          if (stream.messageId !== event.message.id) return stream;
          return {
            ...stream,
            lastUpdatedAt: event.timestamp,
            status: 'ended',
    ...
    const unsubscribeStreamError = engine.on('message:stream:error', (event) => {
      setStreams((current) => {
        const nextStreams = current.map((stream): StreamSnapshot => {
          if (stream.messageId !== event.messageId) return stream;
          return {
            ...stream,
            lastUpdatedAt: event.timestamp,
            status: 'error',
