...
import { InternalPollingTransport } from './internal-polling-transport';
...
export class DitTransport implements ITransport<TransportEvent, TransportEvent> {
  private readonly config: DitTransportConfig;
  private readonly transport: InternalPollingTransport<TransportEvent, TransportEvent>;
  ...
  constructor(config: DitTransportConfig) {
    ...
    this.transport = new InternalPollingTransport({
  ...
  private async pollEvents(): Promise<ReadonlyArray<TransportEvent>> {
  ...
  private async fetchMessages(
    ...
    const json = (await response.json()) as DitFetchMessagesResponse;
    if (!json.success) {
      return [];
    }
    return json.data;
  ...
  private toTransportEvents(
    ...
    return newMessages.map((item) => {
      ...
      this.pruneSeenIds();
      const message = mapDitMessageToCoreMessage(item, {
      ...
      return {
        ...
        timestamp: Date.now(),
      } as const satisfies TransportEvent;
    });
  }
  ...
  private pruneSeenIds(): void {
    ...
    for (let i = 0; i < excess; i++) {
      ...
      if (!oldest.done) this.seenMessageIds.delete(oldest.value);
    }
  }

  private async sendToDit(event: TransportEvent): Promise<void> {
    if (event.type !== 'message' || !this.config.send) {
      return;
    }

    const payload = event.payload as unknown as Partial<Message>;
    ...
    const json = (await response.json()) as DitSendMessageResponse;
    if (!json.success) {
...
export function mapDitMessageToCoreMessage(
  ...
  const timestamp = raw.created_at ? Date.parse(raw.created_at) : Date.now();
  const senderRole =
 ...
 */
export function extractMessageText(payload: Partial<Message>): string | null {
  if (payload.type === 'text' && typeof payload.content === 'string') {
    return payload.content;
  }
  if (payload.type === 'ai' && typeof payload.content === 'string') {
    return payload.content;
  }
  if (payload.type === 'system' && typeof payload.content === 'string') {
    return payload.content;
  }
  return null;
}
...
function sortDitMessagesByCreatedAt(
  ...
  return [...messages].sort((left, right) => {
    const leftTimestamp = Date.parse(left.created_at ?? new Date(0).toISOString());
    const rightTimestamp = Date.parse(right.created_at ?? new Date(0).toISOString());
